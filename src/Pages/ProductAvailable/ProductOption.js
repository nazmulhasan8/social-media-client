...
const ProductOption = ({ productValue, setItemValue }) => {
    ...
    const handleStatusUpdate = id => {
        setLike(like+1);
        fetch(`https://social-media-server-seven.vercel.app/orders4/${id}`, {
            method: 'PATCH',
            headers: {
    ...
    const {data: mongobdUsers = [], refetch} = useQuery({
        ...
        queryFn: async() =>{
            const res = await fetch('https://social-media-server-seven.vercel.app/sellers');
...
const handleReportProduct = productValue => {
    fetch(`https://social-media-server-seven.vercel.app/reporttoadmin`, {
        method: 'POST', 
        headers: {
            'content-type': 'application/json', 
            authorization: `bearer ${localStorage.getItem('accessToken')}`
    ...
    return (
<div className="card card-compact w-80 h-30 bg-fuchsia-500 shadow-xl my-10 mx-36">
...
{refetch()&&<AddComment  key={_id} products2={productValue}></AddComment>}
{refetch()&&<MyComments key={_id} _id={_id}></MyComments>}
